...
class Condition:
    def __init__(self, condition_as_words):
        assert len(condition_as_words) == 3
        self.lhs = condition_as_words[0]
        self.operation = condition_as_words[1]
        self.rhs = condition_as_words[2]

    def evaluate(self, registers):
        rhs_value = self.get_value(self.rhs, registers)
        lhs_value = self.get_value(self.lhs, registers)

        return eval("{} {} {}".format(lhs_value, self.operation, rhs_value))

    def get_value(self, value_string, registers):
        try:
            return int(value_string)
        except ValueError:
            return registers[value_string]

class Instruction:
    def __init__(self, line):
        words = line.split()
        assert len(words) == 7, "input line does not match puzzle description: {}".format(line)
        self.destination_register = words[0]
        self.operation = words[1]
        self.operand = words[2]
        self.condition = Condition(words[4:])

    def execute(self, registers):
        if self.condition.evaluate(registers):
            if self.operation == 'inc':
                registers[self.destination_register] += int(self.operand)
            else:
                registers[self.destination_register] -= int(self.operand)
...
def puzzle1(input_data):
    ...
    instructions = [Instruction(line) for line in input_data.splitlines()]
    for instruction in instructions:
        instruction.execute(registers)
    return max(registers.values())

assert puzzle1(test_input) == 1, "Sanity check of code based on puzzle description failed"
...
assert puzzle2(test_input) == 10, "Sanity check of code based on puzzle description failed"
