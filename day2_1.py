...
class Row:
    def __init__(self, content_string):
        self.values = [int(n) for n in content_string.split()]
    ...
    def find_evenly_divisable(self):
        for dividend in self.values:
            divisor = self.find_even_divisor(dividend)
            if divisor != None:
                return (dividend, divisor)

    def find_even_divisor(self, dividend):
        for divisor in self.values:
...
class Spreadsheet:
    ...
    def __init__(self, content_string):
        self.rows = [Row(line) for line in content_string.splitlines()]


input = '''179	2358	5197	867	163	4418	3135	5049	187	166	4682	5080	5541	172	4294	1397
...
spreadsheet = Spreadsheet(input);
#print(sum([row.max_value() - row.min_value() for row in spreadsheet.rows])) -- Part one
print(sum([row.find_evenly_divisable()[0] / row.find_evenly_divisable()[1] for row in spreadsheet.rows]))