...
# pass phrase is a list of words
pass_phrases = [line.split() for line in input.splitlines()]

# lazy approach to checking for duplicates: If there are no duplicates, a set created from the list of words has the same length as the original set
valid_pass_phrases = [phrase for phrase in pass_phrases if len(phrase) == len(set(phrase))]
print('The answer to problem 1 is {}'.format(len(valid_pass_phrases)))

'abc --> abc, acb, bac, bca, cab, cba'

def get_anagrams(original):
    anagrams = []
    if len(original) == 0:
        return []
    if len(original) == 1:
    ...
    for i in range(len(original)):
        first = original[i]
        rest = original[0:i] + original[i+1 :]
        anagrams.extend([first + ana for ana in get_anagrams(rest)])
    return anagrams

def contains_anagram(sentence):
    for i in range(len(sentence)):
        word = sentence[i]
        rest = sentence[:i] + sentence[i+1:]
        if any(anagram in rest for anagram in get_anagrams(word)):
            return True

valid_pass_phrases_2 = [phrase for phrase in pass_phrases if not contains_anagram(phrase) ]
print('The answer to problem 2 is {}'.format(len(valid_pass_phrases_2)))
