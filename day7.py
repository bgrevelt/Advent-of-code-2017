...
class Program:
    ...
    def total_weight(self):
        return self.weight + sum(self.get_child_weights())
    def get_child_weights(self):
        return [child.total_weight() for child in self.children]
    def is_balanced(self):
        child_weights = self.get_child_weights()
        if len(child_weights) == 0:
            return True
        return all(child_weights[0] == child_weight for child_weight in child_weights)
    def __str__(self):
        s = "{} ({})".format(self.name, self.weight)
        if len(self.children) > 0:
            s += " -> {}".format(' '.join([child.name for child in self.children]))
        s += "\ntotal weight: {}".format(self.total_weight())
        return s

def build_program_dict(input):
    lines = [line.split() for line in input.splitlines()]
    # 'parse' the input to (name, size, [children)
    programs = [ (line[0], int(line[1].strip("()")), [child.strip(',') for child in line[3:]]) if len(line) >= 4 else (line[0], int(line[1].strip("()")), []) for line in lines]
    # First create a dictionary of Program classes without cildren
    program_dict = { name : Program(name,size) for name,size,children in programs}
    # then add the child nodes to the tree
    for name, size, children in programs:
        for child in children:
            program_dict[name].add_child(program_dict[child])
    return program_dict
...
def get_root_node(nodes):
    ...
    for name,node in nodes.items():
        depth = node.depth()
        if depth > root_node[1]:
            root_node = (name, depth)
    return nodes[root_node[0]]


def day7_1(input):
    return get_root_node(build_program_dict(input)).name

assert day7_1(test_input) == 'tknk', "Sanity check failed for puzzle 1"

print("The answer for puzzle 1 is {}".format(day7_1(puzzle_input)))

def depth_first_tree_iterator(node):
    for child in node.children:
        for child_node in depth_first_tree_iterator(child):
            yield(child_node)
    yield(node)

def balance_node(node):
    child_weigths = node.get_child_weights()
    unique_weights = list(set(child_weigths))

    assert len(unique_weights) == 2, "The puzzle rules are that there is only one weight that is wrong. That means there should be two different weights on the disk"
    #The weight that occurs more than once is the right one
    right_weight = unique_weights[0] if child_weigths.count(unique_weights[0]) > 1 else unique_weights[1]
    wrong_weight = unique_weights[0] if child_weigths.count(unique_weights[0]) == 1 else unique_weights[1]

    for child in node.children:
        if child.total_weight() == wrong_weight:
            return child.weight + (right_weight - wrong_weight)

def get_deepest_unbalanced_node(gen):
    deepest = None
    for node in gen:
        if not node.is_balanced() and (deepest == None or deepest[0] > node.depth()):
            deepest = (node.depth(), node)
    return deepest[1]

def day7_2(input):
    dict = build_program_dict(input)
    root = get_root_node(dict)
    # We need to find the deepest node that is unbalanced (as that wil unbalance all nodes above it). Use depth first iteration
    deepest_unbalanced = get_deepest_unbalanced_node(depth_first_tree_iterator(root))
    return balance_node(deepest_unbalanced)

assert day7_2(test_input) == 60, "Sanity check failed for puzzle 1"
print("The answer for puzzle 2 is {}".format(day7_2(puzzle_input)))
