...
class Particle:
    def __init__(self, line):
        parsed = [[int(''.join([c for c in number if c not in "pva=<>"])) for number in unit.split(',')] for unit in line.split(', ')]
        self.position = parsed[0]
        self.velocity = parsed[1]
        self.acceleration = parsed[2]

    def get_acceleration_vector(self):
        return sum([abs(n)for n in self.acceleration])
    ...
    def future_position(self, time):
        position = [0,0,0]
        for axis in range(3):
            position[axis] = int(0.5 * self.acceleration[axis] * time ** 2 + ( 0.5 * self.acceleration[axis] + self.velocity[axis]) * time + self.position[axis])
        return position

puzzle_input = '''p=<2366,784,-597>, v=<-12,-41,50>, a=<-5,1,-2>
...
def puzzle1(puzzle_input):
    ...
    # the idea here is pretty simple. If we are thinking about the particle that is closest by at time = inf, then we basically only care about the acceleration
    min_acceleration = min(particle.get_acceleration_vector() for particle in particles)
    min_accelerating_particles = [particle for particle in particles if particle.get_acceleration_vector() == min_acceleration]
    # However, we may have multiple particles with the same (low) acceleration. In that case, we want the one with the lowest start velocity
    min_velocity = min([particle.get_velocity_vector() for particle in min_accelerating_particles])
    min_velocity_particles = [particle for particle in particles if particle.get_velocity_vector() == min_velocity]
    # And then of course we may end up with tow particles with the same (low) acceleration AND velocity. In that case, we want the one that started out closes to the origin
    min_distance = min([particle.manhattan_distance() for particle in min_velocity_particles])
    min_distance_particles = [particle for particle in particles if particle.manhattan_distance() == min_distance]
    # At this point we may still have multiple particles, but in that case the puzzle does not make sense
    assert len(min_distance_particles) == 1, "There are multiple particles that will stay at the same minimum distance"
    return particles.index(min_distance_particles[0])
...
def get_clash_times(particle1, particle2):
    ...
    p(t) = p(0) + t * v(0) +  t*(t+1) / 2 * A
    or
    0.5A T**2 + (0.5A + V(0)) * T + P(0)
    So if we want to know when two particles clash, we have to solve the equation
    0.5A1 T**2 + (0.5A1 + V(0)1) * T + P(0)1 = 0.5A2 T**2 + (0.5A2 + V(0)2) * T + P(0)2
    or
